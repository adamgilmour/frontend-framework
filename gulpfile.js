...
gulp.task('browserSync', function() {
    browserSync({
        proxy: "localhost/frontend-boilerplate/dist",
        options: {
            reloadDelay: 250
        },
        notify: false
...
gulp.task('phpincludes', function() {
    return gulp.src(path.phpincludes.files) // grab php includes
        .pipe(gulp.dest(path.phpincludes.dest)) // copy to destination folder
        .pipe(browserSync.reload({stream: true})); // notify browserSync to refresh
});

gulp.task('fonts', function() {
    return gulp.src(path.fonts.files) // grab fonts
        .pipe(gulp.dest(path.fonts.dest)) // copy to destination folder
        .pipe(browserSync.reload({stream: true})); // notify browserSync to refresh
});

gulp.task('images', function() {
    return gulp.src(path.images.files) // grab images
...
gulp.task('default', ['deploy', 'browserSync'], function() { // run the 'deploy' task, then start browserSync
    gulp.watch('app/*', ['html']); // watch all html files in the app/ folder and run the 'html' task when any of them changes
    gulp.watch('app/includes/*', ['phpincludes']); // watch all html files in the app/ folder and run the 'html' task when any of them changes
